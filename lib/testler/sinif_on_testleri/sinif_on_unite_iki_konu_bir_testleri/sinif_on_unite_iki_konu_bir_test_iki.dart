...
class SinifOnUniteIkiKonuBirTestIki extends StatefulWidget {
  @override
  _SinifOnUniteIkiKonuBirTestIkiState createState() =>
      _SinifOnUniteIkiKonuBirTestIkiState();
}

class _SinifOnUniteIkiKonuBirTestIkiState
    extends State<SinifOnUniteIkiKonuBirTestIki> {
  List<String> sorular = VeriTabaniSinifOn().soruResimleriOnlu;
  List<String> cevaplar =
      VeriTabaniSinifOn().sinifOnUniteIkiKonuBirTestIkiCevaplar;
            ...
            children: [
              ...
              Column(
                children: [
                  Image(
                    image: AssetImage(
                        'assets/soru_resimleri/soru_resimleri_sinif_on/soruresim_sinif_on_unite_iki_konu_bir_test_iki/${sorular[indexSoru]}'),
                  ),
                ],
              ),
              SizedBox(
  ...
  MathsCatGestureDetector buildMathsCatGestureDetector(
    ...
    return MathsCatGestureDetector(
      onPress: () {
          ...
          if (Provider.of<Puan>(context, listen: false)
            ...
            Provider.of<Puan>(context, listen: false).testBittiMiKaydet(
                Provider.of<Puan>(context, listen: false)
                    .testBittiMiSinifOnUniteIkiKonuBirTestIki = true,
                true,
                'sinif10_5');
          }
        ...
        setState(() {
          if (cevaplar[indexSoru] == label && indexSoru <= 8) {
            soruPuan += 1;
            indexSoru += 1;
          } else {
            if (indexSoru <= 8) {
